...
namespace midisequencer
{
   namespace
   {
      ...
      uint_least32_t read_variable_value(void *fp, int(*fgetc)(void *),
                                         uint_least32_t *track_length, const char *errtext)
      {
         int ret = 0;
         int d;
         do
   ...
   static long buf_idx = 0;
   static int memgetc(void *fp)
   {
      unsigned char *buf = (unsigned char *)fp;

      return buf[buf_idx++];
   }
   bool sequencer::load(void *fp)
   {
      /* reset internal buffer index */
      buf_idx = 0;

      return load(fp, memgetc);
   }
   int sequencer::get_num_ports()const
   {
      int ret = 0;
      for (std::vector<midi_message>::const_iterator i = messages.begin();
            i != messages.end(); ++i)
      {
         if (ret < i->port)
            ret = i->port;
   ...
   void sequencer::play(float time, output *out)
   {
      if (position != messages.begin() && (position - 1)->time >= time)
         position = messages.begin();
      if (position == messages.begin() && position != messages.end()
            && position->time < time)
         out->reset();
   ...
   void sequencer::load_smf(void *fp, int(*fgetc)(void *))
   {
      ...
      {
         if (fgetc(fp) != 0x4D || fgetc(fp) != 0x54 || fgetc(fp) != 0x72
               || fgetc(fp) != 0x6B)
         ...
         for (;;)
         {
            ...
            uint_least32_t delta = read_variable_value(fp, fgetc, &track_length,
                                   "unexpected EOF (deltatime)");
            time += delta;
            if (division & 0x8000)
            {
            ...
            switch (param)
            {
               case 0xF0:
               {
                  int n = read_variable_value(fp, fgetc, &track_length,
                                              "unexpected EOF (sysex length)");
                  std::string s(n + 1, '\0');
                  ...
                  switch (type)
                  {
                     ...
                     case 0x54:
                        ...
                        if (msg.time == 0 && (division & 0x8000))
                        {
                           ...
                           time = 0;
                           time_offset = (hour & 0x1F) * 3600 + min * 60 + sec + (frame + subframe / 100.0)
                                         * fps;
                        }
                        break;
                  }
...
next_track:
      ...
      if (!(division & 0x8000))
      {
         ...
         double time_offset = 0;
         double base = 0;
         for (std::vector<midi_message>::iterator i = messages.begin();
               i != messages.end(); ++i)
         {
...
#include "program.h"
   ...
   void output::midi_message(int port, uint_least32_t message)
   {
      (void)port;
   ...
   void output::sysex_message(int port, const void *data, std::size_t size)
   {
      (void)port;
   ...
   void output::meta_event(int type, const void *data, std::size_t size)
   {
      (void)data;
      (void)size;
   ...
   int output::synthesize(int_least16_t *output, std::size_t samples, float rate)
   {
      return synth->synthesize(output, samples, rate);
   }

   void output::set_mode(midisynth::system_mode_t mode)
   {
